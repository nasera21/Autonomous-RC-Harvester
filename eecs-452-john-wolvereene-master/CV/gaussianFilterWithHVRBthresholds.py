...
for frame in camera.capture_continuous(rawCapture, format="bgr", use_video_port=True):
    ...
    # Box filter
    postBfilterh = cv2.GaussianBlur(diffImh,(3,3),0)
    ...
    # Box filter
    postBfilterv = cv2.GaussianBlur(diffImv,(3,3),0)
    #print(np.shape(postBfilter))
    # Threshold
    vthold_val = 50
    ...
    # Box filter
    postBfilterR = cv2.GaussianBlur(diffImr,(3,3),0)
    #print(np.shape(postBfilter))
    # Threshold
    hthold_val = 50
    ...
    # Box filter
    postBfilterb = cv2.GaussianBlur(diffImb,(3,3),0)
    ...
    contours, hierarchy = cv2.findContours(threshTot,cv2.RETR_LIST, cv2.CHAIN_APPROX_SIMPLE)
    contour = 0
    widthArr = []
    middleArr = []
    hImg, wImg = threshTot.shape
    minArea = 25*25
 ...
 #       box = np.int0(box)
     #   cv2.drawContours(image,[np.int0(box)],(200,0,0),2)
